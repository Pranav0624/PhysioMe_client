...
import './ShoulderSideExtensionAnalysis.css';
import jsPDF from 'jspdf';
import html2canvas from 'html2canvas';
import vnitLogo from './vnit_logo.jpeg';

const ShoulderSideExtensionAnalysis = () => {
  const location = useLocation();
  const { analysisData } = location.state || {};

  const { validReps, invalidReps, efficiencies, name, email, threshold, targetReps, xAxisValue } = analysisData;

  const [armRotation, setArmRotation] = useState(0);

  // Log the xAxisValue for debugging
  useEffect(() => {
    console.log("xAxisValue:", xAxisValue);
    if (xAxisValue !== undefined) {
      setArmRotation(xAxisValue);
    }
  }, [xAxisValue]);

  // Test with static rotation value
  console.log("armRotation:", armRotation); // Log rotation value for debugging
...
export default ShoulderSideExtensionAnalysis;
